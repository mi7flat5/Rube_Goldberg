...
public class ConveyorScript : MonoBehaviour {
	...
	public bool lockOrientationOfCollidingObjects; //When on, this locks the orientation of the object as it collides with the conveyor belt, this makes the object move in a more controlled way when resting on the conveyor.
	...
	private float objectVelocityDifference;
	...
	void Update () {
		...
		//if ( !lockOrientationOfCollidingObjects ) {
		//	foreach (Rigidbody rigidbody in listOfRigidbodiesOnConveyor) {
		//		//objectVelocityDifference = Vector3.SqrMagnitude (rigidbody.velocity - conveyorVelocityVector);
		//			//if (objectVelocityDifference  > 0.0001f ) {
		//			rigidbody.velocity = conveyorVelocityVector;
		//	}
		//}
  ...
  //      if (collision.gameObject.name == "ConveyorBelt(Clone)")
  //          return;
